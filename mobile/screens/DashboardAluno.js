...
export default function DashboardAluno({ route, navigation }) {
  ...
  useEffect(() => {
    async function carregarTurmas() {
      try {
        const response = await axios.get(`http://192.168.224.138:8080/aluno-turma/turmas/${usuario.id}`);
  ...
  const handleVerNotas = async () => {
    try {
      let url;
      if (turmaSelecionada) {
        url = `http://192.168.224.138:8080/entregas/notas/aluno/${usuario.id}/turma/${turmaSelecionada}`;
      } else {
        url = `http://192.168.224.138:8080/entregas/notas/aluno/${usuario.id}`;
      }
  ...
  const handleVerAtividades = async () => {
    ...
    try {
      const response = await axios.get(
        `http://192.168.224.138:8080/entregas/status/aluno/${usuario.id}/turma/${turmaSelecionada}`
      );
